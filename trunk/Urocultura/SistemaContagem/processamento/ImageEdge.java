...
public class ImageEdge {
	 ...
	 * obtained with alpha = 0.5, upper = 100, lower = 50.
	 * 
	 * @param ip
	 *            ImageProcessor
	 * @param alpha
	 *            alpha setting for Canny-Deriche filter
	 * @param upper
	 *            Upper treshold for double tresholding
	 * @param lower
	 *            lower treshold for double tresholding
	 * @return ImageProcessor with 16bit greyscale filtered image
	 * */
	public static ImageProcessor areaEdge(ImageProcessor ip, float alpha,
			float upper, float lower) {
		int type = getType(ip);
		if (type == OTHER) {
			IJ.error("area detection",
					"No action taken. Greyscale or pseudocolored images required!");
			return ip;
		}
		ImageProcessor ipcopy = getDeriche(ip, alpha);
	...
	public static ImageProcessor getDeriche(ImageProcessor ip, float alpha) {
		...
		if (type == OTHER) {
			IJ.error("Deriche",
					"No action taken. Greyscale or pseudocolored image required!");
			return ip;
		}
	 ...
	 * */
	public static ArrayList<double[]> dericheCalc(ImageProcessor ip,
			float alphaD) {
		...
		for (i = 0; i < lignes; ++i) {
			...
			a2[icolonnes + 1] = an1 * a1[icolonnes + 1] + an2 * a1[icolonnes]
					- ad1 * a2[icolonnes];
			for (j = 2; j < colonnes; ++j) {
				a2[icolonnes + j] = an1 * a1[icolonnes + j] + an2
						* a1[icol_1 + j] - ad1 * a2[icol_1 + j] - ad2
						* a2[icol_2 + j];
			}
		}
		...
		for (j = 0; j < colonnes; ++j) {
			...
			for (i = 2; i < lignes; ++i) {
				a3[i * colonnes + j] = an11 * a2[(i - 1) * colonnes + j] - ad1
						* a3[(i - 1) * colonnes + j] - ad2
						* a3[(i - 2) * colonnes + j];
			}
		}
		...
		for (j = 0; j < colonnes; ++j) {
			...
			a4[(lig_2 * colonnes) + j] = -an11 * a2[lig_1 * colonnes + j] - ad1
					* a4[lig_1 * colonnes + j];
			for (i = lig_3; i >= 0; --i) {
				a4[i * colonnes + j] = -an11 * a2[(i + 1) * colonnes + j] - ad1
						* a4[(i + 1) * colonnes + j] - ad2
						* a4[(i + 2) * colonnes + j];
			}
		}
		...
		for (i = 0; i < lignes; ++i) {
			...
			for (j = 2; j < colonnes; ++j) {
				a2[icolonnes + j] = an11 * a1[icol_1 + j] - ad1
						* a2[icol_1 + j] - ad2 * a2[icol_2 + j];
			}
		}
		...
		for (i = 0; i < lignes; ++i) {
			...
			for (j = col_3; j >= 0; --j) {
				a3[icolonnes + j] = -an11 * a1[icol_1 + j] - ad1
						* a3[icol_1 + j] - ad2 * a3[icol_2 + j];
			}
		}
		icol_1 = lignes * colonnes;
		for (i = 0; i < icol_1; ++i) {
		...
		for (j = 0; j < colonnes; ++j) {
			a3[j] = an1 * a2[j];
			a3[colonnes + j] = an1 * a2[colonnes + j] + an2 * a2[j] - ad1
					* a3[j];
			for (i = 2; i < lignes; ++i) {
				a3[i * colonnes + j] = an1 * a2[i * colonnes + j] + an2
						* a2[(i - 1) * colonnes + j] - ad1
						* a3[(i - 1) * colonnes + j] - ad2
						* a3[(i - 2) * colonnes + j];
			}
		}
		...
		for (j = 0; j < colonnes; ++j) {
			a4[lig_1 * colonnes + j] = 0;
			a4[lig_2 * colonnes + j] = an3 * a2[lig_1 * colonnes + j] - ad1
					* a4[lig_1 * colonnes + j];
			for (i = lig_3; i >= 0; --i) {
				a4[i * colonnes + j] = an3 * a2[(i + 1) * colonnes + j] + an4
						* a2[(i + 2) * colonnes + j] - ad1
						* a4[(i + 1) * colonnes + j] - ad2
						* a4[(i + 2) * colonnes + j];
			}
		}
	 ...
	 * @return the image with non local-maxima suppressed
	 */
	public static ImageProcessor nonMaximalSuppression(ImageProcessor grad,
			ImageProcessor ang) {
		FloatProcessor res = new FloatProcessor(grad.getWidth(),
				grad.getHeight());
