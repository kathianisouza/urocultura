...
public class Image_Edge implements PlugIn {
    ...
    private boolean greyImage(ImagePlus img) {
        if (img.getType() < ImagePlus.COLOR_256) {
            return true;
        } else {
            return false;
        }
    ...
    public ImagePlus EdgeDetection(ImagePlus img) {
        ...
        float alpha = (float) SetAlpha;
        float upper = (float) 100;
        float lower = (float) 50;
        ...
        ImageProcessor tmp1;
        ImagePlus areaOutline;

        for (int s = 1; s <= stack.getSize(); s++) {
            tmp1 = ImageEdge.areaEdge(stack.getProcessor(s), alpha, upper,
                    lower);
            result.addSlice("", tmp1);
        }
        areaOutline = new ImagePlus("Area Outline", result);
        //areaOutline.show();
        return areaOutline;
    }

    private void hysteresis(ImagePlus img) {
        boolean show_trin = true;
        GenericDialog gd = new GenericDialog("Parameters");
        ...
        ImageStack stack = img.getStack();
        ImageStack res_trin = new ImageStack(stack.getWidth(),
                stack.getHeight());
        ImageStack res_hyst = new ImageStack(stack.getWidth(),
                stack.getHeight());
     ...
     */
    private void deriche(ImagePlus img) {
        double[] norm_deriche;
        ...
        ImageStack res_norm = new ImageStack(stack.getWidth(),
                stack.getHeight());
        ImageStack res_angle = new ImageStack(stack.getWidth(),
                stack.getHeight());
        ImageStack suppr = new ImageStack(stack.getWidth(), stack.getHeight());

        FloatProcessor tmp = new FloatProcessor(stack.getWidth(),
                stack.getHeight());
        ...
        for (int s = 1; s <= stack.getSize(); s++) {
            ...
            tmp = new FloatProcessor(stack.getWidth(), stack.getHeight(),
                    norm_deriche);
            tmp.resetMinAndMax();
            ...
            suppr.addSlice("", ImageEdge.nonMaximalSuppression(
                    res_norm.getProcessor(s), tmp));
        }
