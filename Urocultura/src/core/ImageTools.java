...
public class ImageTools {
  ...
  public static float[][] getCurrentImageMatrix(float[][] gettypeofA,int c,String directoryImages,String list){
     boolean useRoi = false; // defualt if nothing selected
     return getCurrentImageMatrix(gettypeofA, useRoi,c,directoryImages,list);
  }
  public static float[][] getCurrentImageMatrix(float[][] gettypeofA, boolean useRoi,int c,String directoryImages,String list) {
     int ImgWidth, ImgHeight, ImgTLx, ImgTLy; // bounds of image being retrieved;//altura,largura,coordenadas
		ImagePlus imp;
	 if(c==0){
	      imp = WindowManager.getCurrentImage();//retorna referencia para imagm ativa
	}else{
