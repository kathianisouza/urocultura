...
//     IJ.wait(500); // give system time to initialize GUI
  ...
  public void displayKernel(float[][] kernel) {
     // display kernel 
     if (kernelAd != null) kernelAd.destroy(); // clear old window
     kernelAd = new ArrayDisplay(kernel,"Template");
  ...
  public float[][] cropForKernel(float[][] origimg) {//modificação: este médoto não recebia nada como parâmetro,agora recebe imagem original;
     // get Roi
     ...
     for (int i=0; i<roiWidth; i++)
        for (int j=0; j<roiHeight; j++) 
           newkernel[i][j] = origimg[roiTLx + i][roiTLy + j];// não era conhecida.
     return newkernel;
  } // end cropForKernel
