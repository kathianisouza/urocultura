...
public class FileResults {
	...
	public static void fileHoughCircles(int index, String idImage, String directoryFile, int countCircles, float sigma, float alpha, int threshold){
    	...
    	try{
    		out = new FileWriter(new File(directoryFile + "Pre processamento 1 - " + "S(" + sigma + ") " + "A(" + alpha + ") " + "T(" + threshold + ") " + ".xls"),true);
    		if(index == 0){
	...
	//pre processamento 2
	public static void fileHoughCircles2(int index, String idImage, String directoryFile, int countCircles, float sigma, float alpha, int threshold){
		FileWriter out;
