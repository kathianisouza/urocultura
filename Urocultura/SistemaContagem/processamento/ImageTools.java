...
public class ImageTools {
    ...
    public static float avg(float[][] A, int sx, int sy, int width, int height) {
        if ((sx + width > A.length) || (sy + height > A[0].length)) {
            IJ.showMessage("  Error in Arrays.avg() 2d: out of array bounds");
            return -1f;
        }
        float sum = 0.0f;
...
// linear regression: y = ax + b, assume x=0,1,2,...,N-1, returns values[2]
    public static float linRegrSlope(float[] A) {
        ...
        avgx = sumx / (float) n;
        for (int i = 0; i < n; i++) {
            delsqrx += ((float) i - avgx) * ((float) i - avgx);
        }
        for (int i = 0; i < n; i++) {
            a += A[i] * ((float) i - avgx) / delsqrx;
        }
...
//  resulting corrImage is smaller than original image
    ...
    public static float[][][][] statsCorrelation(float[][][] pixels, float[][][][] kernels) {
        ...
        for (int n = 0; n < nkernels; n++) {
            ...
            for (int i = 0; i < kw; i++) {
                for (int j = 0; j < kh; j++) {
                    for (int k = 0; k < kd; k++) {
                        Syy[n] += sqr(kernels[n][k][i][j] - kernelAvg[n]); //sqr é um método para elevar ao quadrado
                    }
                }
            }
        }
                            ...
                            for (int ww = -ew; ww <= ew; ww++) {
                                for (int hh = -eh; hh <= eh; hh++) {
                                    Sxy[n] += ((float) (pixels[z + dd][x + ww][y + hh]) - localAvg)
                                            * (kernels[n][dd + ed][ww + ew][hh + eh] - kernelAvg[n]);//a parte de cima da equação
                                }
                            }
                        }
                        if (Sxy[n] == 0f) {
                            corr = 0f; // if both Sxy and Sxx == 0, then gives NAN
                        } else {
    ...
    public static float[][] getCurrentImageMatrix(float[][] gettypeofA, ImagePlus imp) {//,int c,String directoryImages,String list){
        boolean useRoi = false; // defualt if nothing selected
        return getCurrentImageMatrix(gettypeofA, useRoi, imp);//,c,directoryImages,list);
    }

    public static float[][] getCurrentImageMatrix(float[][] gettypeofA, boolean useRoi, ImagePlus imp) {//,int c,String directoryImages,String list) {
        int ImgWidth, ImgHeight, ImgTLx, ImgTLy; // bounds of image being retrieved;//altura,largura,coordenadas
        //ImagePlus imp;
//if(c==0){
        //ImagePlus imp = WindowManager.getCurrentImage();//retorna referencia para imagm ativa
/*}else{
        Opener op = new Opener();
        imp = op.openImage(directoryImages + list);
        }*/
        if (imp == null) {
            IJ.noImage();
            return null;
        }
        Roi roi = imp.getRoi();//retorna  a seleção corrente
        if (useRoi && roi != null) { // get just the Roi
            Rectangle roiRect;
            roiRect = roi.getBounds();//retorna um delimitador retangular da seleção
            ImgWidth = roiRect.width;//retorna largura da roi
            ImgHeight = roiRect.height;//retorna altura da roi
            ImgTLx = roiRect.x;
            ImgTLy = roiRect.y; //retorna coordenadas
        } else { // get whole image
            ImgWidth = imp.getWidth();//largura da imagem corrente 
            ImgHeight = imp.getHeight(); // altura da imagem corrente
            ImgTLx = 0;
            ImgTLy = 0; //coordenadas que  no caso começam com 0 pois não existe roi selecionada
        }
        ImageProcessor ip = imp.getProcessor();
        float[][] A = new float[ImgWidth][ImgHeight];
        for (int w = 0; w < ImgWidth; w++) {
            for (int h = 0; h < ImgHeight; h++) {
                A[w][h] = (float) ip.getPixel(w + ImgTLx, h + ImgTLy);//retorna os pixels da imagem corrente
            }
        }
        return A;
    }
...
// with additional parameters.
    ...
    public static Point2D[] getResultTest(ImagePlus imp, int minSize, int maxSize) {
    ...
    public static float[] getArea(ArrayDisplay curAD, int minSize, int maxSize) {
        ImagePlus curImp = curAD.getImagePlus();
        ResultsTable rt = new ResultsTable();
        rt.setPrecision(2 /* int precision */);
        int options = ParticleAnalyzer.SHOW_OUTLINES; // + ParticleAnalyzer.SHOW_OUTLINES;
        int measurements = Measurements.CENTROID + Measurements.MIN_MAX + Measurements.AREA;
        //  ij/measure/Measurements.java
        ParticleAnalyzer pa = new ParticleAnalyzer(options, measurements, rt, minSize, maxSize);
        //  ij/plugin/filter/ParticleAnalyzer.java
        ...
        float[] areaValues = rt.getColumn(rt.AREA);
        for (int i = 0; i < areaValues.length; i++) {
            System.out.println(areaValues[i]);
        }
        return areaValues;

    }

//public static float[] getAreaTeste()
    // elliptical gaussian kernel
    public static float[][] gaussianKernel(int nx, int ny) {
        if (nx % 2 == 0 || ny % 2 == 0) {
            IJ.showMessage("  error in gaussianKernal(2D): must have odd-valued dimensions");
            nx++;
            ny++; // give a value that will work
        }
    ...
    public static float[][] gaussianKernel(float sigmax, float sigmay) {
        ...
        for (i = 0; i < nx; i++) {
            ...
            for (j = 0; j < ny; j++) {
                locy = (float) (j - ((ny - 1) / 2));
                gauss_kernel[i][j] = (float) (exp(-0.5f * (locx * locx / (sigmax * sigmax) + locy * locy / (sigmay * sigmay))));
...
// area = integral{ sqrt(R^2 - x^2)  dx}
    ...
    public static float computePixelArcArea(float Xp1, float Xp2,
        ...
        if (cenX < 0f) {
            ...
            Xp1 = -Xp2;
            Xp2 = -tmpc;
        } // pixel in -x half-plane
        if (cenY < 0f) {
            tmpc = Yp1;
            Yp1 = -Yp2;
            Yp2 = -tmpc;
        } // pixel in -y half-plane
